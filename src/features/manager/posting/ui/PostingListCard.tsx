...
export function PostingListCard({ posting, onClick }: PostingListCardProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'w-full overflow-hidden rounded-2xl bg-white text-left shadow-sm transition-colors hover:bg-bg-light/60',
        isClosed && 'opacity-70'
      )}
    >
      <div className="flex flex-col gap-1.5 px-4 pb-3 pt-4">
        <div className="flex items-start justify-between gap-2">
          <h3
            className={cn(
              'line-clamp-1 typography-body01-semibold',
              isClosed ? 'text-text-70' : 'text-text-100'
            )}
          >
            {posting.title}
          </h3>
