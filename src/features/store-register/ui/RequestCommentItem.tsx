...
import type {
  ...
  WorkspaceRequestCommentFileDto,
} from '@/features/store-register/types/workspaceRequests'

type Props = {
  comment: WorkspaceRequestCommentDto
}
...
function FileChip({
  ...
  file: WorkspaceRequestCommentFileDto
  isAdmin: boolean
}) {
  return (
    <a
...
/** 댓글 한 건 — 관리자(ADMIN) 좌측 회색 / 본인(USER) 우측 main 톤 */
export function RequestCommentItem({ comment }: Props) {
